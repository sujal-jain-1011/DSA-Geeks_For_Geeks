...
/*Function to Solve Two Sum Problem in a Sorted Array*/
void two_sum_sorted(int target)
{
    long long unsigned int i=0, j=size-1;
    while(i<j)
    {
        if(arr[i]+arr[j]==target)
        {
            printf("Sum of %d and %d equals %d", arr[i], arr[j], target);
            return;
        }
    ...
    printf("There are no two elements in the Array whose sum equals %d", target);
}
