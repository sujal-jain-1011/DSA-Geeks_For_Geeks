...
/*Function to implement Binary Search in an Array: Recursive Approach*/
void binary_search(int element, long long signed int low, long long signed int high)
{
    if(low>high)
    {
    ...
    if(arr[mid]==element)
    {
        printf("%d is present in the Array at index:%lld", element, mid);
        return;
    }
...
int main()
{
    ...
    else
    {
        ...
        scanf("%d", &element);
        binary_search(element, 0, size-1);
    }
    return 0;
}
