...
/*Function to Find the Index of First Occurence of an Element-Using Recursive Binary Search*/
void index_first_occurence(int element, long long unsigned int low, long long unsigned int high)
{
    if(low>high)
    {
    ...
    if(arr[mid]==element)
    {
        ...
        {
            printf("The Index of First Occurence of %d is:%llu", element, mid);
            return;
        }
        if(mid!=0)
...
int main()
{
    ...
    else
    {
        ...
        scanf("%d", &element);
        index_first_occurence(element, 0, size-1);
    }
    return 0;
}
