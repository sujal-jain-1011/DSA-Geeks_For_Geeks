...
class Implement_Fixed_Sized_2D_Array{
    ...
    void print_boundary_elements()
    {
        ...
        cout<<"Boundary Elements of Matrix are:"<<endl;
        if(row_size==1)
        {
            for(long long unsigned int i=0; i<column_size; i++)
            cout<<mat[0][i]<<" ";
        }
        ...
        else
        {
            for(long long unsigned int i=0; i<column_size; i++)
            cout<<mat[0][i]<<" ";
            for(long long unsigned int i=1; i<row_size; i++)
            cout<<mat[i][column_size-1]<<" ";
            for(long long unsigned int i=column_size-2; i>=0; i--)
            {
                if(i==0)
                {
                    cout<<mat[row_size-1][i]<<" ";
                    break;
                }
                cout<<mat[row_size-1][i]<<" ";
            }
            for(long long unsigned int i=row_size-2; i>=1; i--)
            cout<<mat[i][0]<<" ";
        }
    }
};
