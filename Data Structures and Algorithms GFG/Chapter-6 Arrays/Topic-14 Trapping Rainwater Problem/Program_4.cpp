...
class Implement_Fixed_Sized_Array{
    ...
    int trapping_rainwater()
    {
        ...
        int *lmax=new (nothrow) int[size];
        int *rmax=new (nothrow) int[size];

        /*Checking for Memory Allocation*/
        if(lmax==NULL)
        {
            cout<<"Memory Allocation Failed for lmax Array!"<<endl;
            return -1;
        }
        else if(rmax==NULL)
        {
            cout<<"Memory Allocation Failed for rmax Array!"<<endl;
            return -1;
        }
        ...
        for(long long signed int i=size-2; i>=0; i--)
        {
...
int main()
{
    ...
    {
        IFSA.display();
        int result=IFSA.trapping_rainwater();
        if(result!=-1)
        cout<<"The Total Volume of Trapped Rainwater is:"<<IFSA.trapping_rainwater();
    }
    return 0;
}