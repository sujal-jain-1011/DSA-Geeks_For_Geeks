...
int max_sum(long long unsigned int sub_size)
{
    ...
    int result=INT_MIN, current_sum;
    for(long long unsigned int i=0; i<=size-sub_size; i++)
    {
        current_sum=0;
        for(long long unsigned int j=i; j<sub_size+i; j++)
        current_sum+=arr[j];
...
int main()
{
    ...
    else
    {
        ...
        scanf("%llu", &sub_size);
        printf("The Maximum Sum that a Subarray of Size %llu can produce is:%d", sub_size, max_sum(sub_size));
    }
    return 0;
}
