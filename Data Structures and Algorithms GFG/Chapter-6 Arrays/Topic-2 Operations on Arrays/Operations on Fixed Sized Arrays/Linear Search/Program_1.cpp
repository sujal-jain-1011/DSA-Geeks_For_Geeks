/*C++ Program to Search for an Element in an
Unsorted Fixed Sized Array(Heap Allocated)*/
...
class Implement_Fixed_Sized_Array{
    ...
    long long unsigned int capacity, size, result_index;
    int *arr;
    int element;
...
int main()
{
    ...
    else
    {
        ...
        cin>>IFSA.element;
        if(!IFSA.search(IFSA.element))
        cout<<IFSA.element<<" is not there in Array!";
        else
        cout<<IFSA.element<<" is there in Array at Index:"<<IFSA.result_index;
    }
    return 0;
}