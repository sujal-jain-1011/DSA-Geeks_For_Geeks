...
class Implement_Fixed_Sized_Array{
    ...
    long long unsigned int maximum_consecutive_1()
    {
        ...
        for(long long unsigned int i=0; i<size; i++)
        {
            long long unsigned int current_result=0;
            for(long long unsigned j=i; j<size; j++)
            {
                if(arr[j]==1)
                current_result++;
                else
...
int main()
{
    ...
    else
    {
        ...
        cout<<"After converting the int Array to bool Array we have Array as:"<<endl;
        IFSA.display();
        cout<<"The Count of Maximum Consecutive 1's in the Array is:"<<IFSA.maximum_consecutive_1();
    }
    return 0;
}