...
class Implement_Fixed_Sized_Array{
    ...
    int maximum_difference()
    {
        ...
        int current_minimum=min(arr[1], arr[0]);
        for(long long unsigned int i=2; i<size; i++)
        {
            result=max(result, (arr[i]-current_minimum));
...
int main()
{
    ...
    {
        IFSA.display();
        cout<<"The Maximum Difference in the Array is:"<<IFSA.maximum_difference()<<endl;
    }
    return 0;
}