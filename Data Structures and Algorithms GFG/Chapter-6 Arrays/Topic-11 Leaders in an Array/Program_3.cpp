/*C++ Program to Print Leaders in a 
...
class Implement_Fixed_Sized_Array{
    ...
    /*Member Function of class:
    to Print Leaders in an Array*/
    void leaders_in_an_array()
    {
        int current_leader=arr[size-1];
        cout<<current_leader<<" ";
        for(long long unsigned int i=size-1; i>0; i--)
        {
            if(arr[i-1]>current_leader)
            {
                current_leader=arr[i-1];
                cout<<current_leader<<" ";
            }
        }
    }
...
int main()
{
    ...
    {
        IFSA.display();
        cout<<"Leader Elements in the Array are:"<<endl;
        IFSA.leaders_in_an_array();
    }
    return 0;
}