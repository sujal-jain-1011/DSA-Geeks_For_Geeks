/*C++ Program to Search for the Largest Element
...
class Implement_Fixed_Sized_Array{
    ...
    Element  in an Array*/
    long long unsigned int largest_element()
    {
        long long unsigned int i, res=0;
        for(i=1; i<size; i++)
        {
            if(arr[i]>arr[res])
            res=i;
...
int main()
{
    ...
    {
        IFSA.display();
        cout<<"Largest Element in the Array is:"<<IFSA.arr[IFSA.largest_element()];
    }
    return 0;
}